echo "make sure run 'kubectl delete ns <StackName>' first', then delete Cloudformation Stack (including manually delete EKS cluster after EKS nodegroups are deleted automatically), at last run below"
if [ $# -ne 2 ]; then
